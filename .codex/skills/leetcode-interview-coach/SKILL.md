...
- Never pass Chinese long text through nested PowerShell, ad hoc command strings, here-strings, pipelines, inline `python -` snippets, or host-decoded Git output. Write UTF-8 JSON/text files at ASCII temp paths, then pass only file paths to scripts.
- For commit/workflow metadata that contains Chinese, use `scripts/write_utf8_metadata.py` with a UTF-8 metadata source directory or `--field-file` inputs. Do not hand-build or modify metadata JSON with PowerShell `Set-Content`, `Out-File`, redirection, here-strings, or pipelines; Windows PowerShell `-Encoding UTF8` may write a BOM and break downstream Python JSON readers. To add generated ASCII fields such as a commit hash, rerun `write_utf8_metadata.py --base-json <old> --set-field commit=<hash> --output <new>`, then pass the new file path.
