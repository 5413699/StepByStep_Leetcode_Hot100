...
FUNCTION_DEFINITIONS: list[dict[str, Any]] = [
    {
        ...
        "commonFunction": "数学Math的常用函数",
        "notePath": "src/notes/03.常用函数/10.数学Math/Math.min.md",
        "signature": "Math.min(int a, int b)",
        "summary": "`Math.min(a, b)` 返回两个数中较小的一个，常用于维护最少次数、最低代价等候选答案。它是静态方法，两个 int 参数对应的返回值也是 int。",
        "whenToUse": [
            "需要在两个候选答案中保留较小值",
            "动态规划枚举多个来源，要不断更新组成当前状态的最少次数或代价",
            "需要把当前最优答案与新候选比较，并把返回值保存回状态变量",
        ],
        "exampleTitle": "完全平方数：枚举最后一个平方数，维护一维 DP 最小值",
        "exampleCode": (
            "int[] dp = new int[n + 1];\n"
            "dp[0] = 0;\n"
            "for (int i = 1; i <= n; i++) {\n"
        ...
        "sources": [
            {
                "label": "Oracle Java Math.min(int, int)",
                "url": "https://docs.oracle.com/en/java/javase/21/docs/api/java.base/java/lang/Math.html#min(int,int)",
                "note": "官方文档定义了静态方法 `min(int a, int b)`：返回两个 int 中较小的值；相等时返回该值。",
            }
        ],
        "patterns": [r"\bMath\s*\.\s*min\s*\("],
    },
    {
        "id": "arrays-fill",
