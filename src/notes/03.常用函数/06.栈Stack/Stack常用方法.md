...
使用 Java `Stack` 保存还没有找到右侧更矮柱子的柱子下标，当前柱子更矮时连续弹出并结算矩形面积。
...
- H084-柱状图中最大的矩形：使用 Java `Stack` 保存还没有找到右侧更矮柱子的柱子下标，当前柱子更矮时连续弹出并结算矩形面积。（题目笔记：`src/notes/01.按数据结构分类/06.栈/H084_柱状图中最大的矩形.md`）
